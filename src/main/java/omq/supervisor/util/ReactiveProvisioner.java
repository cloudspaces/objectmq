...
public class ReactiveProvisioner extends Provisioner {
	...
	//
	// logger.info("Obs param: " + obs + ", Pred param: " + pred +
	// ", NumServersNeeded: " + numServersNeeded + " NumSerNow: " +
	// numServersNow);
	// FileWriterProvisioner.write(new Date(), "REACTIVE", obs, pred,
	// reqArrivalRate, avgServiceTime, varServiceTime, varInterArrivalTime,
	// numServersNeeded, numServersNow);
	// if (diff > 0) {
	...
	private int getNumServersNeeded(double obs, double pred, HasObject[] hasList) throws Exception {
		...
		return (int) Math.ceil(obs / reqArrivalRate);
	}

	// private int getNumServersNeeded(double obs, double pred, HasObject[]
	// hasList) throws Exception {
	// // There are no servers available
	// if (hasList.length == 0) {
	// throw new Exception("Cannot find any server available");
	// }
	...
	// this.varInterArrivalTime = varInterArrivalTime;
	//
	// double reqArrivalRate = 1 / (avgServiceTime + ((varInterArrivalTime +
	// varServiceTime) / (2 * (responseTime - avgServiceTime))));
	//
	// // reqArrival rate is measured in miliseconds but provisioners work with
	// // minutes
	// reqArrivalRate *= sleep;
	//
	// logger.info("ReqArrivalRate: " + reqArrivalRate + " ,AvgServiceTime: " +
	// avgServiceTime + ", VarServiceTime: " + varServiceTime +
	// ", VarInterATime: "
	// + varInterArrivalTime);
	//
	// this.reqArrivalRate = reqArrivalRate;
